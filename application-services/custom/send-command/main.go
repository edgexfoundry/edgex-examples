...
func main() {
	...
	deviceName, err := service.GetAppSetting("DeviceName")
	if err != nil {
		lc.Error(err.Error())
		os.Exit(-1)
	}
	commandName, err := service.GetAppSetting("CommandName")
	if err != nil {
		lc.Error(err.Error())
		os.Exit(-1)
	}
	...
	if err := service.SetFunctionsPipeline(
		functions.NewSwitchCommand(deviceName, commandName).SendSwitchCommand,
